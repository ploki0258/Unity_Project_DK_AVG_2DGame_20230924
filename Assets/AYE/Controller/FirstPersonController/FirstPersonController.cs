...
public class FirstPersonController : MonoBehaviour
{
    ...
    bool isTopSensorOn
    {
        get
        {
            for(int i = 0; i < topSensor.Length; i++)
            {
                if (topSensor[i].on)
                    return true;
            }
    ...
    {
        bool 鎖定 = false;
        if(WindowsManager.instance != null && WindowsManager.instance.AnyWindowsOpen)
            鎖定 = true;
        // wsad
        horizontal = Mathf.Lerp(horizontal, 鎖定? 0f: Input.GetAxisRaw("Horizontal"), wsadMix * Time.deltaTime);;
        vertical = Mathf.Lerp(vertical, 鎖定 ? 0f : Input.GetAxisRaw("Vertical"), wsadMix * Time.deltaTime);
        ...
        rb.velocity = move;
        // mouse
        mouseX += (鎖定 ? 0f : Input.GetAxis("Mouse X")) * mouseSpeed;
        mouseY += (鎖定 ? 0f : Input.GetAxis("Mouse Y"))*-1f * mouseSpeed;
        ...
        if (Input.GetKeyDown(KeyCode.C))
        {
            ...
            if (isDown == false)
                isDown = true;
            else if (isDown && isTopSensorOn == false) // 如果蹲下而且上方沒東西就站起來
            {
                isDown = false;
                isControlDown = false;
            }
        ...
        // 跑步取消蹲下
        if (inputLeftShift && isDown && isTopSensorOn == false)
            isDown = false;
        ...
        // QE
        if (Input.GetKey(KeyCode.Mouse0) && Input.GetKey(KeyCode.Mouse1) == false && 鎖定 == false)
        {
        ...
        else if (Input.GetKey(KeyCode.Mouse1) && Input.GetKey(KeyCode.Mouse0) == false && 鎖定 == false)
        {
